...
import requests # <-- Возвращаем requests
...
def load_json_data(filepath: Path) -> dict | None:
    ...
    except FileNotFoundError: print(f"  ОШИБКА: JSON файл не найден: {filepath}"); return None
    except json.JSONDecodeError: print(f"  ОШИБКА: Не удалось прочитать JSON файл: {filepath}"); return None

def load_source_text(filepath: Path) -> str | None:
    try:
        with open(filepath, 'r', encoding='utf-8') as f: return f.read()
    except FileNotFoundError: print(f"  ОШИБКА: Файл с исходным текстом не найден: {filepath}"); return None
...
def get_structured_context(full_json: dict, path: str) -> dict | None:
    ...
    try:
        ...
        for part in parent_path:
            if isinstance(temp_obj, list) and part.isdigit(): temp_obj = temp_obj[int(part)]
            elif isinstance(temp_obj, dict): temp_obj = temp_obj[part]
        if isinstance(temp_obj, dict): context['local_context'] = temp_obj
        else: context['local_context'] = None
    except (IndexError, KeyError, TypeError):
        context['local_context'] = None
    return context if context.get('local_context') else None

# --- Обновленная функция для взаимодействия с LM Studio ---
def query_lm_studio(messages: list) -> str | None:
    """Отправляет запрос к серверу LM Studio, используя HTTP-запрос."""
    ...
    try:
        ...
        print(f"\n  ОШИБКА СЕТИ: Не удалось подключиться к LM Studio. Убедитесь, что сервер запущен. {e}")
        return None
    except (KeyError, IndexError):
        print(f"\n  ОШИБКА: Неожиданный формат ответа от LLM.")
        return None

# --- Функции для LLM, адаптированные под query_lm_studio ---
def formulate_claim_with_llm(path: str, value, structured_context: dict) -> str | None:
    ...
    print("    -> Этап 1: Отправка запроса на формулировку утверждения...")
...
def validate_claim_with_llm(claim: str, source_text: str) -> dict | None:
    ...
    print("    -> Этап 2: Отправка утверждения 'судье' на проверку...")
    ...
    try:
        ...
        print(f"  ОШИБКА: 'Судья' вернул некорректный JSON: {response_str}"); return None
...
def find_file_pairs(json_dir: Path, ocr_dir: Path) -> list:
    ...
    if not json_dir.is_dir() or not ocr_dir.is_dir():
        print(f"ОШИБКА: Одна из директорий не найдена."); return []
    for json_file in json_dir.glob("*.json"):
        md_file = ocr_dir / f"{json_file.stem}.md"
        if md_file.exists(): pairs.append({"name": json_file.stem, "json_path": json_file, "md_path": md_file})
    return pairs

def validate_file_pair(json_path: Path, source_text_path: Path) -> float:
    print(f"  Загрузка файлов: {json_path.name} и {source_text_path.name}...")
    full_json_data, source_text = load_json_data(json_path), load_source_text(source_text_path)
    if not full_json_data or not source_text: return 0.0
    ...
    if sample_size == 0:
        print("  ПРЕДУПРЕЖДЕНИЕ: Не найдено подходящих полей для валидации."); return 0.0
        
    sampled_points = random.sample(filtered_points, sample_size)
    print(f"  ✓ Выбрано {sample_size} случайных полей для проверки.")

    validation_results, correct_answers_count = [], 0

    for i, (path, value) in enumerate(sampled_points):
        print(f"\n  --- Проверка элемента {i+1}/{sample_size} (путь: {path}) ---")
        
        structured_context = get_structured_context(full_json_data, path)
        if not structured_context:
            print("    ⚠️ Не удалось собрать структурированный контекст, пропускаем."); continue
        
        claim = formulate_claim_with_llm(path, value, structured_context)
        if not claim or '?' in claim or len(claim.split()) < 5:
            print("    ⚠️ Не удалось сформулировать корректное утверждение, пропускаем."); continue
        print(f"    ✅ Сформулированное утверждение: \"{claim}\"")

        llm_verdict = validate_claim_with_llm(claim, source_text)
        if not llm_verdict:
            print("    ⚠️ Не удалось получить вердикт от 'судьи', пропускаем."); continue
        
        validation_results.append({"path": path, "claim": claim, "verdict": llm_verdict})
        is_correct = llm_verdict.get("is_correct", False)
        quote = llm_verdict.get("quote_from_text", "Цитата не найдена.")
        if is_correct:
            correct_answers_count += 1; print(f"    ✅ ВЕРДИКТ: Корректно")
        else:
            print(f"    ❌ ВЕРДИКТ: Некорректно")
        print(f"   💬 Цитата из текста: \"{quote}\"")

    if not validation_results:
        print("  Не было проведено ни одной успешной проверки."); return 0.0
        
    accuracy = (correct_answers_count / len(validation_results)) * 100
    print(f"\n  --- Итог для файла {json_path.name} ---\n  📊 Validation Score: {accuracy:.2f}%")
    return accuracy

def main():
    print("--- Запуск скрипта валидации данных ---")
    file_pairs = find_file_pairs(JSON_DIR, OCR_DIR)
    if not file_pairs:
        print("Не найдено совпадающих пар .json/.md файлов."); return
    ...
    try:
        with open(output_csv_path, 'w', newline='', encoding='utf-8') as csvfile:
            ...
            for i, pair in enumerate(file_pairs):
                print(f"\n{'='*50}\nОбработка пары {i+1}/{len(file_pairs)}: {pair['name']}\n{'='*50}")
                score = validate_file_pair(pair['json_path'], pair['md_path'])
                writer.writerow([pair['name'], f"{score:.2f}"])
        print(f"\n--- ВАЛИДАЦИЯ ЗАВЕРШЕНА ---")
        print(f"✓ Итоговый отчет сохранен в файл: {output_csv_path}")
    except IOError as e: print(f"ОШИБКА: Не удалось записать итоговый файл. {e}")
